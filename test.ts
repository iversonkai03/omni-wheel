...
namespace omni_3wheels {
    // old board pin1pin2 =>> pin 12pin8
    //old board pin13pin14 =>pin16pin0
    //servo pin =>pin4
    //ultra trig=pin6 echo=>pin5
    ...
    let red_btn = 0;
    let green_btn = 0;
    let omni4_right_front = 0;
    let omni4_left_front = 0;
    let omni4_right_back = 0;
    let omni4_left_back = 0;
    ...
    export function omni_3wheels(x: number, y: number, red_btn: number, green_btn: number): void {
        ...
        // motor_moving_newboard(left_wheel_force, right_wheel_force, back_wheel_force);
    ...
    //%block
    export function four_omniwheel_car(x: number, y: number, red_btn: number, green_btn: number): void {
        rotate = 0;
        if (red_btn == 1)
            rotate = 50
        else if (green_btn == 1)
        ...
        omni4_right_front = front_right_force(x, y, rotate);
        omni4_left_front = front_left_force(x, y, rotate);
        omni4_right_back = back_right_force(x, y, rotate);
        omni4_left_back = back_left_force(x, y, rotate);
    ...
    export function motor_moving(L_Wheel: number, R_Wheel: number, B_wheel: number) {
        ...
        if (R_Wheel >= 0) {
            ...
            pins.analogWritePin(AnalogPin.P12, pins.map(right_wheel_force, 0, 31, 0, 1023))
            pins.digitalWritePin(DigitalPin.P8, 0)
        }
        else {
            pins.analogWritePin(AnalogPin.P8, pins.map(right_wheel_force, 0, -31, 0, 1023))
            pins.digitalWritePin(DigitalPin.P12, 0)


        }
        if (L_Wheel >= 0) {
            pins.analogWritePin(AnalogPin.P16, pins.map(left_wheel_force, 0, 31, 0, 1023))
            pins.digitalWritePin(DigitalPin.P0, 0)
        }
        else {


            pins.analogWritePin(AnalogPin.P0, pins.map(left_wheel_force, 0, -31, 0, 1023))
    ...
    export function front_left_force(x: number, y: number, rotate: number): number {

        let temp: number;
        temp = -x * 80 / 100 - y * 80 / 100 + rotate * 35 / 100;


        return temp;

    }
    export function front_right_force(x: number, y: number, rotate: number): number {
    ...
    export function back_left_force(x: number, y: number, rotate: number): number {
    ...
    export function back_right_force(x: number, y: number, rotate: number): number {
